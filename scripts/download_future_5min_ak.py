#!/usr/bin/env python3
"""
期货 5 分钟数据批量下载 - AKShare 版本

无需认证，免费使用
下载 15-20 个活跃品种的 5 分钟主力合约数据（3 年历史）
"""
...
class FutureDataDownloader:
    ...
    def __init__(self, db_path: str = "data/future_data_ak.db"):
        """
        初始化下载器
    ...
    def download_symbol_data(self, symbol: str) -> int:
        ...
        try:
            ...
            if df.empty:
                logger.warning(f"{symbol} 过滤后无数据")
                return 0
    ...
    def download_all(self):
        """下载所有品种数据"""
        logger.info("=" * 80)
        logger.info("期货 5 分钟数据批量下载 (AKShare)")
        logger.info("=" * 80)
        ...
        logger.info(f"数据周期：{self.frequency}分钟")
        logger.info("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='期货 5 分钟数据批量下载 (AKShare)')
    parser.add_argument('--db', type=str, default='data/future_data_ak.db',
                       help='数据库路径')
