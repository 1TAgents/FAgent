...
5. 不虚构数据中不存在的信息
6. 对“最近一周/近一周行情”，A 股默认按最近 5 个交易日的日 K 线处理，不要把自然周误当成 7 根日 K
7. 回答这类行情总结时，必须明确说明：后台使用的工具、查询参数、实际返回的日期范围、数据源/是否实时、以及使用的数据字段
8. 如果工具结果包含逐日 K 线明细，先写清楚“实际数据范围：YYYY-MM-DD 至 YYYY-MM-DD”，再进行走势分析
9. 不要擅自换算成交量单位；除非工具明确给出单位，否则把 volume 说明为原始成交量
10. 不要给日期添加“周一/周二”等星期标签，除非工具结果明确提供或你已准确计算
11. 不要推断用户持有某股票、仓位或成本；除非用户当前消息明确说明，行情总结只分析标的本身
12. 严格区分 volume 和 amount：volume 是原始成交量，amount 是成交额/金额；不要把“50 亿成交额”写成“50 亿成交量”
...
3. **简体中文优先**：FAgent 面向中国大陆用户首发，标题必须尽量使用简体中文。
4. **英文转义**：除股票代码、模型名、产品名、编程语言等必要专有名词外，不要保留英文测试词或内部意图词。
5. **拒绝废话**：严禁包含"关于"、"讨论"、"咨询"、"对话"等无意义词汇。
6. **格式规范**：只输出标题文本，不要加引号、标点或其他符号。
...
- 用户问：capability qa smoke -> 能力问答测试
- 用户问：frontend proxy stream smoke -> 前端流式代理测试
"""
