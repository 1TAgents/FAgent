...
数据源：`~/Learning/quant_repos/data/QuantMInd/feature_snapshots`

标的：`600000` / `SH600000`

区间：2024-01-01 到 2025-12-31，共 485 个交易日

执行参数：`lot_size=1, slippage=0`

| 策略 | 参数 | 总收益 | 买入持有基准 | Alpha | Sharpe | 最大回撤 | 交易次数 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `rsi2` | `period=2, trend_ma=120, buy_below=10, sell_above=65` | -7.08% | 101.33% | -108.41% | -0.33 | -13.37% | 58 |
| `donchian_breakout` | `entry_window=20, exit_window=10` | 43.13% | 101.33% | -58.20% | 1.17 | -14.66% | 17 |
| `sma_trend` | `ma_period=200` | 19.78% | 101.33% | -81.54% | 0.63 | -18.82% | 5 |

结论：

- 三个策略都已经能在当前回测 API、QuantMInd 数据源和向量化执行器上跑通。
- 该样本中买入持有非常强，趋势过滤和突破策略虽然有正收益，但显著跑输基准，因此单看绝对收益不够，必须同时看 `benchmark_return` 和 `alpha`。
- `rsi2` 交易频率高但表现差，说明短线均值回归需要更严格的市场环境过滤、成本假设和样本外验证。
- 下一步优化应优先补 walk-forward 验证报告、参数稳定性汇总和多标的横截面/组合回测能力。
